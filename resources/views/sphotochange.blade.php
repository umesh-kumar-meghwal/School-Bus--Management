...
<head>
    ...
    <title>Student Photo Change</title>
    <!-- Tailwind CSS CDN -->
...
<body class="bg-slate-100 font-sans antialiased min-h-screen flex items-center justify-center p-4">
    ...
    <div class="max-w-md w-full bg-white rounded-2xl border border-slate-200 shadow-xl p-8">
        ...
        <div class="mb-6">
            ...
            <h1 class="text-2xl font-bold text-slate-800">Student Photo Change</h1>
            <p class="text-sm text-slate-500 mt-1">Nayi tasveer select karke pichli photo ko tabdeel (replace) karein.</p>
        </div>

        <!-- Form Start -->
        <form action="/sphotochanges" method="post" enctype="multipart/form-data" class="space-y-6">
            @csrf

            <!-- Styled File Input Area -->
            <div class="space-y-2">
                <label class="block text-sm font-medium text-slate-700">Choose New Photo</label>
                ...
                <div class="flex items-center justify-center w-full">
                    <label class="flex flex-col items-center justify-center w-full h-40 border-2 border-slate-300 border-dashed rounded-xl cursor-pointer bg-slate-50 hover:bg-slate-100/70 transition duration-150">
                        <div class="flex flex-col items-center justify-center pt-5 pb-6">
                            ...
                            <p class="text-xs font-semibold text-slate-500">Click to select new file</p>
            ...
            <!-- Hidden Inputs (Crucial for Backend Replacement Logic) -->
    ...
    <script>
        function updateFileName(input) {
            ...
            if (input.files && input.files[0]) {
                fileNameDisplay.textContent = "New Selected: " + input.files[0].name;
